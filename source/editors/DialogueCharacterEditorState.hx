...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	function addEditorBox()
	{
		...
		UI_mainbox = new FlxUITabMenu(null, tabs2, true);
	...
	function addTypeUI()
	{
		...
		leftCheckbox.callback = function() { character.jsonFile.dialogue_pos = 'left'; updateCharTypeBox(); };

		centerCheckbox = new FlxUICheckBox(leftCheckbox.x, leftCheckbox.y + 40, null, null, "Center", 100);
		centerCheckbox.callback = function() { character.jsonFile.dialogue_pos = 'center'; updateCharTypeBox(); };

		rightCheckbox = new FlxUICheckBox(centerCheckbox.x, centerCheckbox.y + 40, null, null, "Right", 100);
		rightCheckbox.callback = function() { character.jsonFile.dialogue_pos = 'right'; updateCharTypeBox(); };
	...
	function addAnimationsUI()
	{
		...
		var addUpdateButton:FlxButton = new FlxButton(10, idleInputText.y + 30, "Add/Update", function()
		{
			...
			if (character.dialogueAnimations.exists(theAnim))
			{
				...
				if (curSelectedAnim == theAnim) { ghostLoop.playAnim(theAnim); ghostIdle.playAnim(theAnim, true); }
			}
			else
			{
	...
	function addCharacterUI()
	{
		...
		var reloadImageButton:FlxButton = new FlxButton(10, scaleStepper.y + 60, "Reload Image", function() { reloadCharacter(); });
		var loadButton:FlxButton        = new FlxButton(reloadImageButton.x + 100, reloadImageButton.y, "Load Character", function() { loadCharacter(); });
		var saveButton:FlxButton        = new FlxButton(loadButton.x, reloadImageButton.y - 30, "Save Character", function() { saveCharacter(); });
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		{
			if      (sender == scaleStepper) { character.jsonFile.scale          = scaleStepper.value; reloadCharacter(); }
			else if (sender == xStepper)     { character.jsonFile.position[0]    = xStepper.value;     reloadCharacter(); }
			else if (sender == yStepper)     { character.jsonFile.position[1]    = yStepper.value;     reloadCharacter(); }
		}
	}
	...
	{
		if (transitioning) { super.update(elapsed); return; }
		...
		if (!blockInput && !animationDropDown.dropPanel.visible)
		{
			...
			if (FlxG.keys.pressed.Q && camGame.zoom > 0.1) { camGame.zoom -= elapsed * camGame.zoom; if (camGame.zoom < 0.1) camGame.zoom = 0.1; }
			if (FlxG.keys.pressed.E && camGame.zoom < 1)   { camGame.zoom += elapsed * camGame.zoom; if (camGame.zoom > 1)   camGame.zoom = 1;   }
			...
			if (FlxG.keys.justPressed.R) { camGame.zoom = 1; mainGroup.setPosition(0, 0); hudGroup.visible = true; }
