...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		// flixel 6.x: GraphicCursorCross foi removido — desenha o cross manualmente
		cameraFollowPointer = new FlxSprite();
		cameraFollowPointer.makeGraphic(40, 40, FlxColor.TRANSPARENT);
		var px = cameraFollowPointer.pixels;
		for (i in 0...40)
		{
			px.setPixel32(i,  19, 0xFFFFFFFF);
			px.setPixel32(i,  20, 0xFFFFFFFF);
			px.setPixel32(19, i,  0xFFFFFFFF);
			px.setPixel32(20, i,  0xFFFFFFFF);
		}
		cameraFollowPointer.dirty = true;
		...
		UI_characterbox = new FlxUITabMenu(null, tabs2, true);
	...
	function reloadBGs()
	{
		...
		while (i >= 0)
		{
			...
			if (memb != null) { memb.kill(); bgLayer.remove(memb); memb.destroy(); }
			--i;
		}
		bgLayer.clear();
		...
		if (onPixelBG)
		{
			...
			if (char.isPlayer) { playerXDifference += 200; playerYDifference = 220; }
			...
			bgSky.setGraphicSize(widShit);    bgSky.updateHitbox();
			bgSchool.setGraphicSize(widShit); bgSchool.updateHitbox();
			bgStreet.setGraphicSize(widShit); bgStreet.updateHitbox();
			bgTrees.setGraphicSize(Std.int(widShit * 1.4)); bgTrees.updateHitbox();
	...
	function addCharacterUI()
	{
		...
		var saveCharacterButton:FlxButton = new FlxButton(reloadImage.x, noAntialiasingCheckBox.y + 40, "Save Character", function() { saveCharacter(); });
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		{
			if (sender == healthIconInputText) { leHealthIcon.changeIcon(healthIconInputText.text); char.healthIcon = healthIconInputText.text; updatePresence(); }
			else if (sender == imageInputText) { char.imageFile = imageInputText.text; }
		}
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (sender == positionXStepper)          { char.positionArray[0]    = positionXStepper.value;      char.x = char.positionArray[0] + OFFSET_X + 100; updatePointerPos(); }
			else if (sender == positionYStepper)          { char.positionArray[1]    = positionYStepper.value;      char.y = char.positionArray[1];                   updatePointerPos(); }
			else if (sender == positionCameraXStepper)    { char.cameraPosition[0]   = positionCameraXStepper.value; updatePointerPos(); }
			else if (sender == positionCameraYStepper)    { char.cameraPosition[1]   = positionCameraYStepper.value; updatePointerPos(); }
			else if (sender == healthColorStepperR)       { char.healthColorArray[0] = Math.round(healthColorStepperR.value); healthBarBG.color = FlxColor.fromRGB(char.healthColorArray[0], char.healthColorArray[1], char.healthColorArray[2]); }
			else if (sender == healthColorStepperG)       { char.healthColorArray[1] = Math.round(healthColorStepperG.value); healthBarBG.color = FlxColor.fromRGB(char.healthColorArray[0], char.healthColorArray[1], char.healthColorArray[2]); }
			else if (sender == healthColorStepperB)       { char.healthColorArray[2] = Math.round(healthColorStepperB.value); healthBarBG.color = FlxColor.fromRGB(char.healthColorArray[0], char.healthColorArray[1], char.healthColorArray[2]); }
		}
	}
	...
	function reloadCharacterImage()
	{
		...
		if (char.animationsArray != null && char.animationsArray.length > 0)
		{
			for (anim in char.animationsArray)
			{
				if (anim.indices != null && anim.indices.length > 0)
					char.animation.addByIndices(anim.anim, anim.name, anim.indices, "", anim.fps, anim.loop);
				else
		...
		if (lastAnim != '') char.playAnim(lastAnim, true); else char.dance();
	...
	function genBoyOffsets():Void
	{
		...
		while (i >= 0)
		{
			...
			if (memb != null) { memb.kill(); dumbTexts.remove(memb); memb.destroy(); }
			--i;
		}
		dumbTexts.clear();
	...
	function loadChar(isDad:Bool, blahBlahBlah:Bool = true)
	{
		...
		while (i >= 0)
		{
			...
			if (memb != null) { memb.kill(); charLayer.remove(memb); memb.destroy(); }
			--i;
		}
		charLayer.clear();
	...
	function updatePointerPos()
	{
		...
		if (!char.isPlayer) { x += 150 + char.cameraPosition[0]; } else { x -= 100 + char.cameraPosition[0]; }
	...
	function reloadAnimationDropDown()
	{
		...
		for (anim in char.animationsArray) { anims.push(anim.anim); ghostAnims.push(anim.anim); }
	...
	function reloadGhost()
	{
		...
		{
			if (anim.indices != null && anim.indices.length > 0)
				ghostChar.animation.addByIndices(anim.anim, anim.name, anim.indices, "", anim.fps, anim.loop);
			else
				ghostChar.animation.addByPrefix(anim.anim, anim.name, anim.fps, anim.loop);
		...
		char.alpha = 0.85;
		ghostChar.visible = true;
		if (ghostDropDown.selectedLabel == '') { ghostChar.visible = false; char.alpha = 1; }
			...
			if (FileSystem.exists(directory))
			{
				for (file in FileSystem.readDirectory(directory))
				{
					...
					if (!sys.FileSystem.isDirectory(path) && file.endsWith('.json'))
					{
						...
						if (!charsLoaded.exists(charToCheck)) { characterList.push(charToCheck); charsLoaded.set(charToCheck, true); }
					}
				}
			}
		}
	...
	override function update(elapsed:Float)
	{
		if (char.animationsArray[curAnim] != null)
		{
			...
			if (curAnimObj == null || curAnimObj.frames.length < 1) textAnim.text += ' (ERROR!)';
		}
		else
		{
			textAnim.text = '';
		...
		for (i in 0...inputTexts.length)
		{
			if (inputTexts[i].hasFocus)
			{
				...
				if (FlxG.keys.justPressed.ENTER) inputTexts[i].hasFocus = false;
				FlxG.sound.muteKeys       = [];
		...
		if (!charDropDown.dropPanel.visible)
		{
			if (FlxG.keys.justPressed.ESCAPE)
			{
				...
				else { MusicBeatState.switchState(new editors.MasterEditorMenu()); FlxG.sound.playMusic(Paths.music('freakyMenu')); }
			...
			{
				var addToCam:Float = 500 * elapsed * (FlxG.keys.pressed.SHIFT ? 4 : 1);
				if (FlxG.keys.pressed.I) camFollow.y -= addToCam;
				if (FlxG.keys.pressed.K) camFollow.y += addToCam;
				if (FlxG.keys.pressed.J) camFollow.x -= addToCam;
				if (FlxG.keys.pressed.L) camFollow.x += addToCam;
			}
	...
	{
		if (prefix.toLowerCase().endsWith('v')) prefix = prefix.substring(0, prefix.length - 1);
