...
class ChartingState extends MusicBeatState
{
		...
		var clear_notes:FlxButton  = new FlxButton(320, clear_events.y + 30, 'Clear notes', function()
		{
			for (sec in 0..._song.notes.length)
			{
				...
				while (count < _song.notes[sec].sectionNotes.length)
				{
					...
					if (note != null && note[1] > -1) _song.notes[sec].sectionNotes.remove(note); else count++;
				}
			}
			updateGrid();
		});
			...
			if (FileSystem.exists(directory))
			{
				for (file in FileSystem.readDirectory(directory))
				{
					...
					if (!FileSystem.isDirectory(path) && file.endsWith('.json'))
					{
						...
						if (!tempMap.exists(stageToCheck)) { tempMap.set(stageToCheck, true); stages.push(stageToCheck); }
					}
				}
			}
		}
	...
	function addSectionUI():Void
	{
		...
		stepperSectionBPM.value = check_changeBPM.checked ? _song.notes[curSection].bpm : Conductor.bpm;
		...
		var pasteButton:FlxButton = new FlxButton(10, 180, "Paste Section", function()
		{
			...
			{
				var copiedNote:Array<Dynamic> = note[4] != null
					? [note[0] + addToTime, note[1], note[2], note[3], note[4]]
					: [note[0] + addToTime, note[1], note[2], note[3]];
		...
		var swapSection:FlxButton = new FlxButton(10, 240, "Swap section", function()
		{
			for (i in 0..._song.notes[curSection].sectionNotes.length)
			{
				...
				if (note[1] > -1) { note[1] = (note[1] + 4) % 8; _song.notes[curSection].sectionNotes[i] = note; }
			}
			updateGrid();
		});
		...
		var copyLastButton:FlxButton = new FlxButton(10, 270, "Copy last section", function()
		{
			...
			for (note in _song.notes[daSec - value].sectionNotes)
			{
				...
				var copiedNote:Array<Dynamic> = note[1] < 0
					? [strum, note[1], note[2], note[3], note[4]]
					: [strum, note[1], note[2], note[3]];
	...
	function addNoteUI():Void
	{
		...
		#if LUA_ALLOWED
		var luaNoteDirectories:Array<String> = [Paths.mods('custom_notetypes/'), Paths.mods(Paths.currentModDirectory + '/custom_notetypes/')];
		for (i in 0...luaNoteDirectories.length)
		{
			var directory:String = luaNoteDirectories[i];
			if (FileSystem.exists(directory))
			{
				for (file in FileSystem.readDirectory(directory))
				{
	...
	function addEventsUI():Void
	{
		...
		var luaEventDirectories:Array<String> = [Paths.mods('custom_events/'), Paths.mods(Paths.currentModDirectory + '/custom_events/')];
		for (i in 0...luaEventDirectories.length)
		{
			var directory:String = luaEventDirectories[i];
			if (FileSystem.exists(directory))
			{
				for (file in FileSystem.readDirectory(directory))
				{
		...
		eventDropDown = new FlxUIDropDownMenuCustom(20, 50, FlxUIDropDownMenuCustom.makeStrIdLabelArray(leEvents, true), function(pressed:String)
		{
			...
			if (curSelectedNote != null) { curSelectedNote[2] = eventStuff[selectedEvent][0]; updateGrid(); }
		});
	...
	function addChartingUI()
	{
		...
		{
			if (vocals != null) vocals.volume = check_mute_vocals.checked ? 0 : 1;
		};
	...
	function generateSong()
	{
		...
		FlxG.sound.music.onComplete = function()
		{
			...
			if (vocals != null) { vocals.play(); vocals.pause(); vocals.time = 0; }
			changeSection();
			curSection = 0;
			updateGrid();
			updateSectionUI();
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			switch (wname)
			{
				...
				case 'note_susLength':
					if (curSelectedNote != null && curSelectedNote[1] > -1) { curSelectedNote[2] = nums.value; updateGrid(); }
					else sender.value = 0;
				case 'section_bpm':
		...
		else if (id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText))
		{
			...
			{
				if (sender == value1InputText)         { curSelectedNote[3] = value1InputText.text; updateGrid(); }
				else if (sender == value2InputText)    { curSelectedNote[4] = value2InputText.text; updateGrid(); }
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.mouse.justPressed)
		{
			if (FlxG.mouse.overlaps(curRenderedNotes))
			{
				curRenderedNotes.forEach(function(note:Note)
				{
					...
					{
						if (FlxG.keys.pressed.CONTROL) selectNote(note); else deleteNote(note);
					}
				});
			}
			else
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.Z && curZoom > 0)                      { --curZoom; updateZoom(); }
			...
			if (FlxG.keys.justPressed.SPACE)
			{
				...
				{
					if (vocals != null) { vocals.play(); vocals.pause(); vocals.time = FlxG.sound.music.time; vocals.play(); }
			...
			{
				FlxG.sound.music.pause();
				var holdingShift:Float = FlxG.keys.pressed.CONTROL ? 0.25 : FlxG.keys.pressed.SHIFT ? 4 : 1;
			...
			if (FlxG.keys.justPressed.LEFT  || FlxG.keys.justPressed.A)
				changeSection(curSection <= 0 ? _song.notes.length - 1 : curSection - shiftThing);
		}
		else if (FlxG.keys.justPressed.ENTER)
		{
			for (i in 0...blockPressWhileTypingOn.length)
				if (blockPressWhileTypingOn[i].hasFocus) blockPressWhileTypingOn[i].hasFocus = false;
		}
		...
		curRenderedNotes.forEachAlive(function(note:Note)
		{
			...
			if (note.strumTime <= Conductor.songPosition)
			{
				...
				if (note.strumTime > lastConductorPos && ((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)) && FlxG.sound.music.playing && note.noteData > -1)
				{
					...
					if (!playedSound[data])
					{
						var soundToPlay = _song.player1 == 'gf' ? 'GF_' + Std.string(data + 1) : 'ChartingTick';
	...
	{
		if (audioBuffers[0] != null) { audioBuffers[0].dispose(); audioBuffers[0] = null; }
		#if MODS_ALLOWED
		...
		if (audioBuffers[1] != null) { audioBuffers[1].dispose(); audioBuffers[1] = null; }
		#if MODS_ALLOWED
	...
	function reloadGridLayer()
	{
		...
		#if desktop
		if (waveformEnabled != null) updateWaveform();
		#end
	...
	function resetSection(songBeginning:Bool = false):Void
	{
		updateGrid();
		FlxG.sound.music.pause();
		FlxG.sound.music.time = songBeginning ? 0 : sectionStartTime();
		if (songBeginning) curSection = 0;
		if (vocals != null) { vocals.pause(); vocals.time = FlxG.sound.music.time; }
		updateCurStep();
		updateGrid();
		updateSectionUI();
	...
	function updateNoteUI():Void
	{
		if (curSelectedNote != null)
		{
			...
			else
			{
				...
				if (selected > 0 && selected < eventStuff.length) descText.text = eventStuff[selected][1];
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSection].sectionNotes)
		{
			...
			if (note.sustainLength > 0) curRenderedSustains.add(setupSusNote(note));
		...
		if (curSection < _song.notes.length - 1)
		{
			for (i in _song.notes[curSection + 1].sectionNotes)
			{
				...
				if (note.sustainLength > 0) nextRenderedSustains.add(setupSusNote(note));
			}
		}
	}
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		if (daNoteInfo > -1)
		{
			if (!Std.isOfType(i[3], String)) i[3] = noteTypeIntMap.get(i[3]);
	...
	function selectNote(note:Note):Void
	{
		...
		for (i in _song.notes[curSection].sectionNotes)
		{
			if (i.length > 2 && i[0] == note.strumTime && i[1] == noteDataToCheck) { curSelectedNote = i; break; }
		}
		updateGrid();
		updateNoteUI();
	}
	...
	private function addNote():Void
	{
		...
		if (noteData > -1)
		{
			_song.notes[curSection].sectionNotes.push([noteStrum, noteData, 0, noteTypeIntMap.get(daType)]);
		}
		else
		{
			var event = eventStuff[Std.parseInt(eventDropDown.selectedId)][0];
			_song.notes[curSection].sectionNotes.push([noteStrum, noteData, event, value1InputText.text, value2InputText.text]);
		}
		curSelectedNote = _song.notes[curSection].sectionNotes[_song.notes[curSection].sectionNotes.length - 1];

		if (FlxG.keys.pressed.CONTROL && noteData > -1)
			_song.notes[curSection].sectionNotes.push([noteStrum, (noteData + 4) % 8, 0, noteTypeIntMap.get(daType)]);
	...
	function clearEvents()
	{
		for (sec in 0..._song.notes.length)
		{
			...
			{
				var note:Array<Dynamic> = _song.notes[sec].sectionNotes[count];
				if (note != null && note[1] < 0) _song.notes[sec].sectionNotes.remove(note); else count++;
			}
		}
		updateGrid();
	}
	...
	private function saveEvents()
	{
		...
		for (sec in 0..._song.notes.length)
		{
			...
			for (i in 0..._song.notes[sec].sectionNotes.length)
			{
				...
				if (note != null && note[1] < 0) arrayNotes.push(note);
			}
			events.push({sectionNotes: arrayNotes, lengthInSteps: 16, typeOfSection: 0, mustHitSection: false, bpm: 0, changeBPM: false, altAnim: false});
		}

		var eventsSong:SwagSong = {
			song: _song.song, notes: events, bpm: _song.bpm,
			needsVoices: _song.needsVoices, speed: _song.speed,
			arrowSkin: _song.arrowSkin, splashSkin: _song.splashSkin,
			player1: _song.player1, player2: _song.player2, player3: _song.player3,
			stage: _song.stage, validScore: false
		};

		var data:String = Json.stringify({"song": eventsSong}, "\t");
		if (data != null && data.length > 0)
		{
			_file = new FileReference();
			_file.addEventListener(Event.COMPLETE,       onSaveComplete);
