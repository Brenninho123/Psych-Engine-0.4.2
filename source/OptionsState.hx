...
class OptionsState extends MusicBeatState
{
	...
	function openDataFolder():Void
	{
		...
		try
		{
			...
			var newFile = JNI.createMemberMethod(
				"java/io/File",
				"<init>",
				"(Ljava/lang/String;)V"
			);
			var uriFromFile = JNI.createStaticMethod(
				"android/net/Uri",
				"fromFile",
				"(Ljava/io/File;)Landroid/net/Uri;"
			);
			var newIntent = JNI.createMemberMethod(
				"android/content/Intent",
				"<init>",
				"(Ljava/lang/String;Landroid/net/Uri;)V"
			);
			var addFlags = JNI.createMemberMethod(
				"android/content/Intent",
				"addFlags",
			...
			var uri      = uriFromFile(file);
			var intent   = newIntent("android.intent.action.VIEW", uri);
			addFlags(intent, 0x10000000); // FLAG_ACTIVITY_NEW_TASK
...
class NotesSubstate extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		for (i in 0...ClientPrefs.arrowHSV.length)
		{
			...
			switch (i)
			{
				case 0: note.animation.addByPrefix('idle', 'purple0');
				case 1: note.animation.addByPrefix('idle', 'blue0');
				case 2: note.animation.addByPrefix('idle', 'green0');
				case 3: note.animation.addByPrefix('idle', 'red0');
			}
	...
	override function update(elapsed:Float)
	{
		if (changingNote)
		{
			if (holdTime < 0.5)
			{
				if (controls.UI_LEFT_P)        { updateValue(-1); FlxG.sound.play(Paths.sound('scrollMenu')); }
				else if (controls.UI_RIGHT_P)  { updateValue(1);  FlxG.sound.play(Paths.sound('scrollMenu')); }
				else if (controls.RESET)       { resetValue(curSelected, typeSelected); FlxG.sound.play(Paths.sound('scrollMenu')); }
			...
			else
			{
				...
				if (controls.UI_LEFT_R || controls.UI_RIGHT_R) { FlxG.sound.play(Paths.sound('scrollMenu')); holdTime = 0; }
			}
		}
		else
		{
			if (controls.UI_UP_P)    { changeSelection(-1); FlxG.sound.play(Paths.sound('scrollMenu')); }
			if (controls.UI_DOWN_P)  { changeSelection(1);  FlxG.sound.play(Paths.sound('scrollMenu')); }
			if (controls.UI_LEFT_P)  { changeType(-1);      FlxG.sound.play(Paths.sound('scrollMenu')); }
			if (controls.UI_RIGHT_P) { changeType(1);       FlxG.sound.play(Paths.sound('scrollMenu')); }
			if (controls.RESET)
			{
		...
		for (i in 0...grpNotes.length)
		{
			...
			item.x = FlxMath.lerp(item.x, intendedPos + (curSelected == i ? 100 : 0), lerpVal);
			for (j in 0...3)
			{
				var item2 = grpNumbers.members[(i * 3) + j];
				item2.x = item.x + 265 + (225 * (j % 3)) - (30 * item2.lettersArray.length) / 2;
				if (ClientPrefs.arrowHSV[i][j] < 0) item2.x -= 20;
			}
			if (curSelected == i) hsvText.setPosition(item.x + hsvTextOffsets[0], item.y - hsvTextOffsets[1]);
		}
		...
		if (controls.BACK || (changingNote && controls.ACCEPT))
		{
			...
			if (!changingNote)
			{
				grpNumbers.forEachAlive(function(spr:Alphabet)  { spr.alpha = 0; });
				grpNotes.forEachAlive(function(spr:FlxSprite)   { spr.alpha = 0; });
				close();
			}
			changingNote = false;
			FlxG.sound.play(Paths.sound('cancelMenu'));
	...
	function changeSelection(change:Int = 0)
	{
		...
		for (i in 0...grpNumbers.length)
		{
			grpNumbers.members[i].alpha = ((curSelected * 3) + typeSelected == i) ? 1 : 0.6;
		}
		for (i in 0...grpNotes.length)
		{
			var item = grpNotes.members[i];
			item.alpha = (curSelected == i) ? 1 : 0.6;
			item.scale.set(curSelected == i ? 1.2 : 1, curSelected == i ? 1.2 : 1);
			if (curSelected == i) hsvText.setPosition(item.x + hsvTextOffsets[0], item.y - hsvTextOffsets[1]);
		}
		FlxG.sound.play(Paths.sound('scrollMenu'));
	}
	...
	{
		typeSelected = (typeSelected + change + 3) % 3;
		curValue = ClientPrefs.arrowHSV[curSelected][typeSelected];
		updateValue();
		for (i in 0...grpNumbers.length)
			grpNumbers.members[i].alpha = ((curSelected * 3) + typeSelected == i) ? 1 : 0.6;
	...
	{
		curValue += change;
		var max:Float = (typeSelected == 1 || typeSelected == 2) ? 100 : 180;
		curValue = Math.max(-max, Math.min(max, curValue));
...
class ControlsSubstate extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		for (i in 0...optionShit.length)
		{
			...
			if (isCentered) { optionText.screenCenter(X); optionText.forceX = optionText.x; optionText.yAdd = -55; }
			else              optionText.forceX = 200;
			optionText.yMult   = 60;
			optionText.targetY = i;
			grpOptions.add(optionText);

			if (!isCentered) addBindTexts(optionText);
		}
		changeSelection();
	}
	...
	override function update(elapsed:Float)
	{
		if (rebindingKey < 0)
		{
			...
			if (controls.BACK)
			{
				...
				for (i in 0...grpInputs.length) if (grpInputs[i] != null) grpInputs[i].alpha = 0;
				close();
				FlxG.sound.play(Paths.sound('cancelMenu'));
			}
			...
			if (controls.ACCEPT && nextAccept <= 0)
			{
				...
				else
				{
					...
					if (rebindingKey > -1) { grpInputs[rebindingKey].visible = false; FlxG.sound.play(Paths.sound('scrollMenu')); }
		...
		else
		{
			...
			if (keyPressed > -1)
			{
				...
				if (controlArray[opposite] == controlArray[rebindingKey]) controlArray[opposite] = NONE;
				reloadKeys();
				FlxG.sound.play(Paths.sound('confirmMenu'));
				rebindingKey = -1;
			}
	...
	function changeAlt()
	{
		...
		for (i in 0...grpInputs.length)
			if (grpInputs[i].sprTracker == grpOptions.members[curSelected])
				grpInputs[i].alpha = (grpInputs[i].isAlt == curAlt) ? 1 : 0.6;
		FlxG.sound.play(Paths.sound('scrollMenu'));
	}

	private function unselectableCheck(num:Int, ?checkDefaultKey:Bool = false):Bool
	{
		if (optionShit[num] == defaultKey) return checkDefaultKey;
		for (i in 0...ClientPrefs.keyBinds.length)
			if (ClientPrefs.keyBinds[i][1] == optionShit[num]) return false;
		return true;
	}
	...
	private function getSelectedKey():Int
	{
		...
		for (i in 0...ClientPrefs.keyBinds.length)
			if (ClientPrefs.keyBinds[i][1] == optionShit[curSelected]) return i * 2 + altValue;
		return -1;
	}
	...
	function reloadKeys()
	{
		...
		for (i in 0...grpOptions.length)
			if (!unselectableCheck(i, true)) addBindTexts(grpOptions.members[i]);
...
class PreferencesSubstate extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		for (i in 0...options.length)
		{
			...
			optionText.isMenuItem = true;
			if (isCentered) { optionText.screenCenter(X); optionText.forceX = optionText.x; }
			else              { optionText.x += 300; optionText.forceX = 300; }
			optionText.yMult   = 90;
			optionText.targetY = i;
			grpOptions.add(optionText);
			...
			if (!isCentered)
			{
				...
				for (j in 0...noCheckbox.length) if (options[i] == noCheckbox[j]) { useCheckbox = false; break; }
		...
		for (i in 0...options.length) if (!unselectableCheck(i)) { curSelected = i; break; }
		changeSelection();
		reloadValues();
	}
	...
	override function update(elapsed:Float)
	{
		...
		{
			grpOptions.forEachAlive(function(spr:Alphabet) { spr.alpha = 0; });
			grpTexts.forEachAlive(function(spr:AttachedText) { spr.alpha = 0; });
			for (i in 0...checkboxArray.length) if (checkboxArray[i] != null) checkboxArray[i].alpha = 0;
			if (showCharacter != null) showCharacter.alpha = 0;
			descText.alpha = 0;
			close();
			FlxG.sound.play(Paths.sound('cancelMenu'));
		}
		...
		for (i in 0...noCheckbox.length) if (options[curSelected] == noCheckbox[i]) { usesCheckbox = false; break; }
		...
		if (usesCheckbox)
		{
			if (controls.ACCEPT && nextAccept <= 0)
			{
				switch (options[curSelected])
				{
					case 'FPS Counter':          ClientPrefs.showFPS            = !ClientPrefs.showFPS; if (Main.fpsVar != null) Main.fpsVar.visible = ClientPrefs.showFPS;
					case 'Low Quality':          ClientPrefs.lowQuality         = !ClientPrefs.lowQuality;
					case 'Anti-Aliasing':
						...
						for (i in 0...checkboxArray.length) if (checkboxArray[i] != null) checkboxArray[i].antialiasing = ClientPrefs.globalAntialiasing;
						OptionsState.menuBG.antialiasing = ClientPrefs.globalAntialiasing;
					case 'Note Splashes':        ClientPrefs.noteSplashes       = !ClientPrefs.noteSplashes;
					case 'Flashing Lights':      ClientPrefs.flashing           = !ClientPrefs.flashing;
					case 'Violence':             ClientPrefs.violence           = !ClientPrefs.violence;
					case 'Swearing':             ClientPrefs.cursing            = !ClientPrefs.cursing;
					case 'Downscroll':           ClientPrefs.downScroll         = !ClientPrefs.downScroll;
					case 'Middlescroll':         ClientPrefs.middleScroll       = !ClientPrefs.middleScroll;
					case 'Ghost Tapping':        ClientPrefs.ghostTapping       = !ClientPrefs.ghostTapping;
					case 'Camera Zooms':         ClientPrefs.camZooms           = !ClientPrefs.camZooms;
					case 'Hide HUD':             ClientPrefs.hideHud            = !ClientPrefs.hideHud;
					case 'Persistent Cached Data': ClientPrefs.imagesPersist    = !ClientPrefs.imagesPersist; FlxGraphic.defaultPersist = ClientPrefs.imagesPersist;
					case 'Hide Song Length':     ClientPrefs.hideTime           = !ClientPrefs.hideTime;
				}
				FlxG.sound.play(Paths.sound('scrollMenu'));
				reloadValues();
			}
		...
		else
		{
			if (controls.UI_LEFT || controls.UI_RIGHT)
			{
				...
				{
					case 'Framerate':
						ClientPrefs.framerate = Std.int(Math.max(60, Math.min(240, ClientPrefs.framerate + add)));
						if (ClientPrefs.framerate > FlxG.drawFramerate) { FlxG.updateFramerate = ClientPrefs.framerate; FlxG.drawFramerate = ClientPrefs.framerate; }
						else { FlxG.drawFramerate = ClientPrefs.framerate; FlxG.updateFramerate = ClientPrefs.framerate; }
					case 'Note Delay':
						var mult:Int = holdTime > 1.5 ? 2 : 1;
						ClientPrefs.noteOffset = Std.int(Math.max(0, Math.min(500, ClientPrefs.noteOffset + add * mult)));
				}
				reloadValues();
		...
		if (showCharacter != null && showCharacter.animation.curAnim.finished) showCharacter.dance();
		if (nextAccept > 0) nextAccept -= 1;
		super.update(elapsed);
	}
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch (options[curSelected])
		{
			case 'Framerate':           daText = "Pretty self explanatory, isn't it?\nDefault value is 60.";
			case 'Note Delay':          daText = "Changes how late a note is spawned.\nUseful for preventing audio lag from wireless earphones.";
			case 'FPS Counter':         daText = "If unchecked, hides FPS Counter.";
			case 'Low Quality':         daText = "If checked, disables some background details,\ndecreases loading times and improves performance.";
			case 'Persistent Cached Data': daText = "If checked, images loaded will stay in memory\nuntil the game is closed, this increases memory usage,\nbut basically makes reloading times instant.";
			case 'Anti-Aliasing':       daText = "If unchecked, disables anti-aliasing, increases performance\nat the cost of the graphics not looking as smooth.";
			case 'Downscroll':          daText = "If checked, notes go Down instead of Up, simple enough.";
			case 'Middlescroll':        daText = "If checked, hides Opponent's notes and your notes get centered.";
			case 'Ghost Tapping':       daText = "If checked, you won't get misses from pressing keys\nwhile there are no notes able to be hit.";
			case 'Note Splashes':       daText = "If unchecked, hitting \"Sick!\" notes won't show particles.";
			case 'Flashing Lights':     daText = "Uncheck this if you're sensitive to flashing lights!";
			case 'Camera Zooms':        daText = "If unchecked, the camera won't zoom in on a beat hit.";
			case 'Hide HUD':            daText = "If checked, hides most HUD elements.";
			case 'Hide Song Length':    daText = "If checked, the bar showing how much time is left\nwill be hidden.";
		}
		...
		for (item in grpOptions.members)
		{
			...
			if (!unselectableCheck(bullShit - 1))
			{
				...
				for (j in 0...checkboxArray.length)
					if (checkboxArray[j].sprTracker == item) { checkboxArray[j].alpha = item.alpha; break; }
			}
		}
		for (i in 0...grpTexts.members.length)
		{
			var text:AttachedText = grpTexts.members[i];
			if (text != null) text.alpha = (textNumber[i] == curSelected) ? 1 : 0.6;
		}
	...
	function reloadValues()
	{
		for (i in 0...checkboxArray.length)
		{
			...
			if (checkbox != null)
			{
				...
				{
					case 'FPS Counter':           daValue = ClientPrefs.showFPS;
					case 'Low Quality':           daValue = ClientPrefs.lowQuality;
					case 'Anti-Aliasing':         daValue = ClientPrefs.globalAntialiasing;
					case 'Note Splashes':         daValue = ClientPrefs.noteSplashes;
					case 'Flashing Lights':       daValue = ClientPrefs.flashing;
					case 'Downscroll':            daValue = ClientPrefs.downScroll;
					case 'Middlescroll':          daValue = ClientPrefs.middleScroll;
					case 'Ghost Tapping':         daValue = ClientPrefs.ghostTapping;
					case 'Swearing':              daValue = ClientPrefs.cursing;
					case 'Violence':              daValue = ClientPrefs.violence;
					case 'Camera Zooms':          daValue = ClientPrefs.camZooms;
					case 'Hide HUD':              daValue = ClientPrefs.hideHud;
					case 'Persistent Cached Data': daValue = ClientPrefs.imagesPersist;
					case 'Hide Song Length':      daValue = ClientPrefs.hideTime;
				}
		...
		for (i in 0...grpTexts.members.length)
		{
			...
			if (text != null)
			{
				...
				{
					case 'Framerate':   daText = '' + ClientPrefs.framerate;
					case 'Note Delay':  daText = ClientPrefs.noteOffset + 'ms';
				}
	...
	{
		for (i in 0...unselectableOptions.length) if (options[num] == unselectableOptions[i]) return true;
		return options[num] == '';
	}
}
